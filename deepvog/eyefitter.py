...
class SingleEyeFitter(object):
    def __init__(
        ...
        # initialisation
        # reserved for numpy array (2,1). Centre coordinate in numpy indexing
        # frame.
    ...
    def unproject_single_observation(
        self, prediction, mask=None, threshold=0.5
    ):
        ...
        # you will have to slice it manually as Y[:,:,1] as input to this
        # function.
        try:
        ...
        # We unproject the gaze vectors and pupil centre only if an ellipse has
        # been detected
        if ellipse_info is not None:

            (rr, cc, centre, w, h, radian, ell) = ellipse_info
            ellipse_confidence = computeEllipseConfidence(
                prediction, centre, w, h, radian
            )

            # Convert centre coordinates from numpy indexing frame to camera
            # frames
            centre_cam = centre.copy()
            centre_cam[0] = centre_cam[0] - self.image_shape[1] / 2
            centre_cam[1] = centre_cam[1] - self.image_shape[0] / 2

            # Convert ellipse parameters to the coefficients of the general
            # form of ellipse equation
    ...
    def fit_projected_eye_centre(
        ...
        # Normalisation of the 2D projection of gaze vectors is done inside
        # intersect()
        if ransac == True:
            # Assuming 20% of outliners
            samples_to_fit = np.ceil(a.shape[0] / 5).astype(np.int)
