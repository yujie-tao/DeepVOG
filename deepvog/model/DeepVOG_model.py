...
def encoding_block(
    X, filter_size, filters_num, layer_num, block_type, stage, s=1, X_skip=0
):
    ...
    for i in np.arange(layer_num) + 1:
        ...
        X = Conv2D(
            ...
            strides=(s, s),
            padding="same",
            name=conv_name_base + "main_" + str(i),
...
def decoding_block(
    ...
    for i in np.arange(layer_num) + 1:
        ...
        X_joined_input = BatchNormalization(
            axis=3, name=bn_name_base + "main_" + str(i)
        )(X_joined_input)
        if i != layer_num:
    ...
    # Up-sampling layer. At the output layer, up-sampling is disabled and
    # replaced by other stuffs manually
    if up_sampling == True:
        X_uped = Conv2DTranspose(
            filters_num,
            (2, 2),
            strides=(2, 2),
            padding="valid",
            name=conv_name_base + "up",
            kernel_initializer=glorot_uniform(),
        )(X_joined_input)
...
def DeepVOG_net(input_shape=(1080, 1980, 3), filter_size=(3, 3)):
