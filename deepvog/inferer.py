...
class gaze_inferer(object):
    def __init__(
        self, model, flen, ori_video_shape, sensor_size, infer_gaze_flag=True
    ):
        """
        ...
        try:
            ...
            assert (
                isinstance(ori_video_shape, tuple)
                or isinstance(ori_video_shape, list)
                or isinstance(ori_video_shape, np.ndarray)
            )
            assert (
                isinstance(sensor_size, tuple)
                or isinstance(sensor_size, list)
                or isinstance(sensor_size, np.ndarray)
            )
            assert (
                isinstance(sensor_size, tuple)
                or isinstance(sensor_size, list)
        ...
        self.flen = flen
        self.ori_video_shape, self.sensor_size = (
            np.array(ori_video_shape).squeeze(),
            np.array(sensor_size).squeeze(),
        )
        self.mm2px_scaling = np.linalg.norm(
            self.ori_video_shape
        ) / np.linalg.norm(self.sensor_size)

        self.model = model
        self.confidence_fitting_threshold = 0.96
        self.eyefitter = SingleEyeFitter(
            focal_length=self.flen * self.mm2px_scaling,
    ...
    def process(
        ...
        """
        # Get video information (path strings, frame reader, video's
        # shapes...etc)
        (
            video_name_root,
            ext,
        ...
        ) = self._get_video_info(video_src)
        (vid_m, vid_w, vid_h, vid_channels) = vid_shapes

        self.vid_manager = VideoManager(
            vreader=vreader,
        ...
        self.eyefitter.focal_length = (
            self.flen * self.mm2px_scaling * image_scaling_factor
        )
        self.eyefitter.pupil_radius = (
            2 * self.mm2px_scaling * image_scaling_factor
        )
        ...
        for idx, frame in enumerate(vreader.nextFrame()):
            ...
            print(
                "\r%s%s %s (%d/%d)"
                % (print_prefix, mode, video_name_root + ext, idx + 1, vid_m),
                end="",
                flush=True,
            )
            ...
            if ((mini_batch_idx != 0) and (idx < final_batch_idx)) or (
                idx == 0
            ):
                X_batch[mini_batch_idx, :, :, :] = frame_preprocessed

            # After reaching the batch size, but not the final batch, predict
            # heatmap and fit/infer angles
            elif (
                (mini_batch_idx == 0)
                and (idx < final_batch_idx)
                or (idx == final_batch_idx)
            ):
                Y_batch = self.model.predict(X_batch)
                if mode == "Fit":
                    self._fitting_batch(X_batch=X_batch, Y_batch=Y_batch)
                elif mode == "Infer":
                    _, _, _ = self._infer_batch(
                        X_batch=X_batch,
                        Y_batch=Y_batch,
            ...
            # Within the final batch and reaching the last index, predict
            # heatmap and fit/infer angles
                ...
                if mode == "Fit":
                    ...
                    _, _, _ = self._infer_batch(
                        X_batch=X_batch_final,
                        Y_batch=Y_batch,
                        idx=idx - final_batch_size,
                    )

        if mode == "Fit":
            # Fit eyeball models. Parameters are stored as internal attributes
            # of Eyefitter instance.
            _ = self.eyefitter.fit_projected_eye_centre(
                ransac=True, max_iters=100, min_distance=10 * vid_m
            )
            _, _ = self.eyefitter.estimate_eye_sphere()

            # Issue error if eyeball model still does not exist after fitting.
            if (self.eyefitter.eye_centre is None) or (
                self.eyefitter.aver_eye_radius is None
            ):
                raise TypeError(
                    "Eyeball model was not fitted. You may need -v or -m argument to check whether the pupil segmentation works properly."
                )
        print()
    ...
    def save_eyeball_model(self, path):
        ...
        if (self.eyefitter.eye_centre is None) or (
            ...
            print(
                "3D eyeball model not found. You may need -v or -m argument to check whether the pupil segmentation works properly."
            )
            raise
        else:
            save_dict = {
    ...
    def load_eyeball_model(self, path):
        ...
        if (self.eyefitter.eye_centre is not None) or (
            self.eyefitter.aver_eye_radius is not None
        ):
    ...
    def _fitting_batch(self, X_batch, Y_batch):
        ...
        if self.vid_manager.output_video_flag:
            # Convert video frames to 8 bit integer format for drawing the
            # output video frames
            video_frames_batch = np.around(X_batch * 255).astype(int)
            vid_frame_shape_2d = (
                video_frames_batch.shape[1],
                video_frames_batch.shape[2],
            )
        ...
        for batch_idx, (X_each, Y_each) in enumerate(zip(X_batch, Y_batch)):
            ...
            if centre is not None:
                ...
                # Draw ellipse and pupil centre on input video if visualization
                # is enabled
                if self.vid_manager.output_video_flag:
                    ellipse_centre_np = np.array(centre)

                    # Draw pupil ellipse
                    vid_frame = draw_ellipse(
                        output_frame=vid_frame,
                        frame_shape=vid_frame_shape_2d,
                        ellipse_info=ellipse_info,
                        color=[255, 255, 0],
                    )
                    # Draw small circle at the ellipse centre
                    vid_frame = draw_circle(
                        output_frame=vid_frame,
                        frame_shape=vid_frame_shape_2d,
                        centre=ellipse_centre_np,
                        radius=5,
                        color=[0, 255, 0],
                    )

                    self.vid_manager.write_frame_with_condition(
                        vid_frame=vid_frame, pred_each=pred_each
                    )
            else:
    ...
    def _infer_batch(self, X_batch, Y_batch, idx):
        ...
        for batch_idx, (X_each, Y_each) in enumerate(zip(X_batch, Y_batch)):
            ...
            # If ellipse fitting is successful, i.e. an ellipse is located, AND
            # gaze inference is ENABLED
            if (centre is not None) and self.infer_gaze_flag:
                ...
                if self.vid_manager.output_video_flag:
                    ...
                    ellipse_centre_np = np.array(centre)
                    projected_eye_centre = reproject(
                        self.eyefitter.eye_centre, self.eyefitter.focal_length
                    )  # shape (2,1)
                    ...
                    projected_eye_centre += (
                        np.array(vid_frame_shape_2d).T.reshape(-1, 1) / 2
                    )

                    vid_frame = self._draw_vis_on_frame(
                        vid_frame,
                        vid_frame_shape_2d,
                        ellipse_info,
                        ellipse_centre_np,
    ...
    def _draw_vis_on_frame(
        ...
        # Draw from eyeball centre to ellipse centre (just connecting two
        # points)
        vec_with_length = ellipse_centre_np - projected_eye_centre.squeeze()
        vid_frame = draw_line(
            output_frame=vid_frame,
            frame_shape=vid_frame_shape_2d,
            o=projected_eye_centre,
            l=vec_with_length,
            color=[0, 0, 255],
        )
